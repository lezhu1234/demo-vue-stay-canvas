...
type ListenerPropsCallback = (p: ActionCallbackProps) => { [key: string]: any } | any
...
export const FileChangeListener: ListenerProps = {
  ...
  state: ALLSTATE,
  callback: ({
    tools: { appendChild, forceUpdateCanvas, getChildrenBySelector, removeChild },
    payload
  }) => {
    ...
    appendChild({
      ...
      shape: new StayImage({
        ...
        height: 0,
        imageLoaded: (self) => {
          const { rectangle } = container.computeFitInfo(self.naturalWidth!, self.naturalHeight!)
          self.update({
            x: rectangle.x,
            y: rectangle.y,
...
export const BrushSizeChangeListener: ListenerProps = {
  ...
  callback: ({ e, store, tools: { updateChild } }) => {
    ...
    store.set('brushSize', Math.max(1, (store.get('brushSize') || 1) + changeSize))
    const indicator = store.get('indicator')
    if (indicator) {
      updateChild({
        child: indicator,
